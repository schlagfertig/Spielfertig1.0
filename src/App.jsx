...
".eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImhzdHdobXF3eG12bG9idnlnc3R5Iiwicm9sZSI6ImFub24iLCJpYXQiO
...
try {
const res = await fetch("https://api.anthropic.com/v1/messages",{ method:"POST", header
headers:{"Content-Type":"application/json","x-api-key":ANTHROPIC_KEY,"anthropic-versi
...
const res = await fetch("https://api.anthropic.com/v1/messages",{ method:"POST", headers:{"Content-Type":"application/json","x-api-key":ANTHROPIC_KEY,"anthropic-versi
header
body:JSON.stringify({ model:"claude-sonnet-4-20250514", max_tokens:4000, system, mess
...
<SongRow song={song} pos={song.position} draggable isDragging={dragId===song.id
...
setSaving(true);
const newPos=mySongs.filter(ps=>ps.set_name===newSet&&ps.playlist_id===play
await sb.update("playlist_songs",{set_name:newSet,position:newPos},"id=eq."
await onRefresh(); setSaving(false);
}}
style={{background:"#111",border:"1px solid #333",color:C.gray,borderRadius:4
>
